...
int main(int argc, char** argv) {
    ...
    for (int i=1;i<argc;i++){
        ...
        if (take("--nx=")) nx=std::atoi(eq+1);
        else if (take("--ny=")) ny=std::atoi(eq+1);
        else if (take("--px=")) px=std::atoi(eq+1);
        else if (take("--py=")) py=std::atoi(eq+1);
        else if (take("--steps=")) steps=std::atoi(eq+1);
        else if (take("--halo=")) halo=std::atoi(eq+1);
    }

    int dims[2] = {px, py};
    MPI_Dims_create(world_size, 2, dims);
    int periods[2] = {0,0};
    MPI_Comm cart; MPI_Cart_create(MPI_COMM_WORLD, 2, dims, periods, 0, &cart);
    int coords[2]; int cart_rank=0; MPI_Comm_rank(cart, &cart_rank);
    ...
    if (world_rank == 0) {
        std::printf("ranks,Px,Py,nx,ny,nx_local,ny_local,steps,halo,total_max,total_min,perstep_worst,rss_kb_max\n");
        std::printf("%d,%d,%d,%d,%d,%d,%d,%d,%d,%.6f,%.6f,%.6f,%zu\n",
            world_size, dims[0], dims[1], nx, ny, nx_local, ny_local, steps, halo,
            total_max, total_min, total_avg, rss_max);
        std::fflush(stdout);
    }
