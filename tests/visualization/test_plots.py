...
def test_compare_fields_vmin_vmax_and_diff_vlim(tmp_path):
    A = np.array([[0, 2], [1, 3]])
    B = np.array([[1, 3], [0, 4]])
...
def test_compare_fields_no_diff_branch(tmp_path):
    A = np.zeros((2, 2))
    B = np.ones((2, 2))
    fig, axes = plots.compare_fields(A, B, show_diff=False, vmin=None, vmax=None)
    plt.close(fig)

def test_compare_fields_overlay_only(tmp_path):
    A = np.zeros((2, 2))
    B = np.ones((2, 2))
    fig, axes = plots.compare_fields(A, B, overlay_minmax=True, show_diff=False)
    plt.close(fig)

def test_animate_from_outputs_empty_steps_raises(monkeypatch):
    monkeypatch.setattr(plots, "list_available_steps", lambda d: [])
    with pytest.raises(RuntimeError):
        plots.animate_from_outputs("dummy_dir")
...
def test_animate_from_outputs_save_pillow(monkeypatch, tmp_path):
    steps = [0]
    frame = np.zeros((2, 2))
    monkeypatch.setattr(plots, "list_available_steps", lambda d: steps)
    monkeypatch.setattr(plots, "load_global", lambda d, s, var="u": frame)
    ...
    out = tmp_path / "anim.gif"
    anim, fig, ax = plots.animate_from_outputs("dummy_dir", steps=steps, save=out, writer="pillow")
    assert isinstance(anim, FuncAnimation)
    plt.close(fig)
...
def test_animate_from_outputs_save_ffmpeg(monkeypatch, tmp_path):
    steps = [0]
    frame = np.zeros((2, 2))
    monkeypatch.setattr(plots, "list_available_steps", lambda d: steps)
    monkeypatch.setattr(plots, "load_global", lambda d, s, var="u": frame)
