...
def test_load_rank_layout_empty_file(tmp_path):
    csv_file = tmp_path / "rank_layout.csv"
    csv_file.write_text("rank,x_offset,y_offset,nx_local,ny_local\n")
    assert cli.load_rank_layout(tmp_path) is None


def test_cmd_show_uses_last_step(monkeypatch):
    monkeypatch.setattr(cli, "list_available_steps", lambda d: [1, 2, 3])
    monkeypatch.setattr(cli, "load_global", lambda *a, **k: [[1]])
    monkeypatch.setattr(cli, "imshow_field", lambda *a, **k: "shown")
...
def test_cmd_compare(monkeypatch):
    monkeypatch.setattr(cli, "load_global", lambda d, s, var="u": [[1]])
    called = {}
    def fake_compare(A, B, **kw):
        called["called"] = True
        return "fig"
    monkeypatch.setattr(cli, "compare_fields", fake_compare)
...
def test_main_with_no_args(monkeypatch):
    ...
    monkeypatch.setattr(cli, "cmd_show", lambda args: setattr(args, "_called", True))
    cli.main(None)